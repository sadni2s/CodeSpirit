 ...
 *
 */
public interface User_AldaRepository extends JpaRepository<User_Alda, Integer> {

    // SELECT firstname, lastname, id
    // FROM User p
    // WHERE p.occupation = [StringValueOf( occupation )]
    List<User_AldaDTO> getUserByOccupation(String occupation );

    // SELECT firstname, lastname, id
    // FROM User p
    // WHERE p.userid = [StringValueOf( userid )] AND p.password = [StringValueOf( password )]
    User_AldaDTO findUserByUseridAndPassword (String userid , String password);
